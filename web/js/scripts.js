...
// navigation links ------------------
	...
	$(".inner a , .inner .logo, .body-overlay").click( function() {
		setTimeout( function(){	
			hidemenu();				 	
		},900);	
	});
...
// google map styles and functions  --------
	...
	var map;
	// your coordinates   -------- 
	var cashemir = new google.maps.LatLng(4.666762, -74.059182);	
	function initialize() {
	  $("#fname").val('');
      $("#femail").val('');
      $("#ftext").val('');
	...
	var companyImage = new google.maps.MarkerImage('images/markerpp.png',
		new google.maps.Size(80,100),
		new google.maps.Point(0,0)
	);
	// your marker coordinates   --------
	var companyPos = new google.maps.LatLng(4.666762, -74.059182);
	...
	function resizeCanvas(mobileState){
		var altMenu = 0;
		if(mobileState!=''){
			altMenu = $('.nav-overlay').height();
		}
		$('#canvas').css('height', ($(window).height()-altMenu));
		$('#canvas .caption div').css('max-height', ($(window).height()-40-altMenu));
	}
	...
	$('#canvas .caption .close').click( function(event) {
		event.preventDefault();
		$('#canvas .caption div').not('.customNavigation').css('display', 'none');
	});
	$('.acanvas').click(function(event){
		$('#suramerica, #clientes').hide();
	})
	$('.aclientes').click(function(event){
		$('#suramerica').hide();
		$('#clientes').show();
	})
    $('.asuramerica').click(function(event){
		$('#clientes').hide();
		$('#suramerica').show();
	})
}